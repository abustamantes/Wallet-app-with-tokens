...
class Wallet(table):
    ...
    def Redeem(self):
        if not self.EMD.get():
            ...
            try:
                ...
                self.Balance.config(text=f"Enter your ID to redeem token, balance is {self.balance}")
                print(f"Enter your ID to redeem token, balance is {self.balance}")
    ...
    def My_ID(self):
        if not self.ID.get():
            print("Introduce your student ID")            
        else:
    ...
    def Target_ID(self):
        if not self.ID2.get():
            print("Introduce the target student ID")            
        else:
