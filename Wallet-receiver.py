...
class Wallet(table):
    ...
    balance=0
    balance_rcv=0
    synch="Your wallet is yet to synchronized"
    Kwallet=""
    student_id=""
    ...
    def Redeem(self):
        try:
            ...
            self.balance=self.decrypt_emd(enc,self.Kwallet)
            self.Balance.config(text=f"Your balance is: {self.balance}")
        except:
            self.Balance.config(text=f"Introduce the token to redeem and your student ID, balance is {self.balance}")
    ...
    def redeem_tk(self):
        if self.redeem_token1.get() not in self.rcv_token:
            ...
            try:
                ...
                if str(self.r_wid_local)==self.wid_local and self.y==0:
                    ...
                    print("Error: The token is incorrect or the transfer is not for this wallet")
            except:
    ...
    def generate_token(self):
        try:
            ...
            self.transfer_token1.config(text="Introduce local and remote WID, and money to transfer")
